...
public class BootCxfMojo extends AbstractMojo {
    ...
    private static final String DATAPI_NOT_FOUND_ERROR_MESSAGE = ".datapi-File not found - is it placed somewhere under /src/main/resources or /src/test/resources?";
    ...
    public void execute() throws MojoExecutionException {
        ...
        List<File> datapi = findDatapi(mavenProject.getBasedir());
		ServicioDataPiBuilder.getInstance().setGlobalpaths(datapi);
		ServicioDataPiBuilder.getInstance().setLog(getLog());
		ServicioDataPiBuilder.getInstance().setLOG_PREFIX(LOG_PREFIX);
		ServicioDataPiBuilder.getInstance().startUp();
		Map<String, ServicioDataPi> servicios = ServicioDataPiBuilder.getInstance().getListadoServicios();
    ...
    protected List<File> findDatapi(File buildDirectory) throws MojoExecutionException {
        String[] extension = {"conf"};
        Collection<File> datapi = FileUtils.listFiles(buildDirectory, extension, true);

        filterOutWsdlsInsideBuildOutputFolder(datapi);

        List<File> datapis = datapi.stream().collect(toList());

        if(!datapis.isEmpty()) {
            return datapis;
        } else {
            throw new MojoExecutionException(DATAPI_NOT_FOUND_ERROR_MESSAGE);
        }
    }
    ...
    protected String generatePackageNameFromTargetNamespaceInWsdl(String targetNamespaceFromWsdl) throws MojoExecutionException {
         ...
         * for example, your package will be org.ada.namespace.weatherservice.
